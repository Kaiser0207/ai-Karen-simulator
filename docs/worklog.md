...
### UI 收尾 + 音量/載入修正(2026-06-10 續)
- **操作說明彈窗**:首次進關卡自動彈一次(`localStorage.okeke_seen_howto`),之後設定「📖 操作說明」可再看;從設定開→按鈕「← 返回設定」並退回設定,關卡自動彈→「開始挑戰 →」單純關閉。
- **語音語氣泡泡標籤**:語音送出後在「你」泡泡下加常駐彩色藥丸(怒=紅/喜=綠/中性=灰/焦慮=黃)→ demo 時每句語氣留在聊天紀錄,跟奧客反應一一對照(打字無語氣、不顯示)。
- **設定緊湊**:操作說明 + 關閉併成一排、留白/間距收緊 → 設定面板不再需要卷軸。
- **靜態版本號** `?v=N`:`index.html` 對 `app.js/style.css` 帶版本參數,改前端只要 bump 版本就抓新檔 → 不再吃瀏覽器舊快取(demo 不怕卡到舊版)。
